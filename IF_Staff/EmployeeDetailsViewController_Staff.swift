...
class EmployeeDetailsViewController_Staff: UIViewController, NSURLConnectionDataDelegate {
    var flag = 0
    var serverData:Data?
    ...
    override func viewWillAppear(_ animated: Bool) {
        print(RequestDetailsViewController.selectedUserid)
        if flag == 1{
            for i in recArray!{
                
                var gd = i as! NSMutableDictionary
                print("WOW")
                if RequestDetailsViewController.selectedUserid == gd.value(forKey: "userid") as! String{
